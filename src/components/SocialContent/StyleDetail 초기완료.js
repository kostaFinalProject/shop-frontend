...
const StyleDetail = () => {
    ...
    const [isCommentVisible, setIsCommentVisible] = useState(false); // 댓글 창 표시 상태
     ...
     // 예제 댓글 데이터 (DB에서 가져온다고 가정)
     const comments = [
        { author: "User1", text: "좋은 글이에요!" },
        { author: "User2", text: "정말 멋진 사진입니다." },
        { author: "User3", text: "어디서 구매했는지 알 수 있을까요?어디서 구매했는지 알 수 있을까요?어디서 구매했는지 알 수 있을까요?" },
    ];
    ...
    return (
        <>
            ...
            {/* 댓글 창 */}
            <StyleComment  isVisible={isCommentVisible} onClose={toggleComments} comments={comments}/>
        </>
