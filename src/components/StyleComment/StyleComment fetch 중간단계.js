...
const StyleComment = ({ isVisible, onClose, comments, setComments, currentUser }) => {
        ...
        if (newComment.trim() !== "") {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    setComments([...comments, data]); // 서버에서 반환된 댓글 추가
    ...
    const handleAddReply = async (index) => {
        if (reComment.trim() !== "") {
            ...
            const newReply = {
                author: currentUser || "대댓글", // 대댓글 작성자
                text: reComment, // 대댓글 내용
                formattedDate: new Date().toLocaleDateString('ko-KR', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit'
            ...
            updatedComments[index].replies.push(newReply); // 대댓글 추가
    
            try {
                // 대댓글 서버에 전송 (예시 API 엔드포인트)
                const response = await fetch(`/api/v1/comments/${updatedComments[index].id}/replies`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (response.ok) {
                    setComments(updatedComments); // 부모 컴포넌트 상태 업데이트
